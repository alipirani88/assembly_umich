    ...
    for file in list_of_files:
        ...
        if args.reference:
            ...
            if args.ariba:
                ...
                command = "module load perl-modules\n/nfs/esnitkin/bin_group/anaconda2/bin//python pipeline.py -f1 %s -f2 %s -o %s/%s -start_step 1 -end_step 4 -A spades -reference %s -type PE -analysis %s -config %s %s %s" % (
                first_file, second_file, args.out_dir, first_part, args.reference, first_part, config, assembly_para,
                ariba_para)
        else:
            ...
            if args.ariba:
                ariba_para = "-ariba " + args.ariba
                command = "module load perl-modules\n/nfs/esnitkin/bin_group/anaconda2/bin//python pipeline.py -f1 %s -f2 %s -o %s/%s -start_step 1 -end_step 4 -A spades -type PE -analysis %s -config %s %s %s" % (
                first_file, second_file, args.out_dir, first_part, first_part, config, assembly_para,
                ariba_para)
        # if args.ariba:
        #     ariba_para = "-ariba " + args.ariba
