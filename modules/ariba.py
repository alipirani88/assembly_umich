...
def ariba_AMR(forward_paired, reverse_paired, output_folder, prefix, logger, Config):
    ...
    ariba_cmd = "/nfs/esnitkin/bin_group/anaconda3/bin/python %s/%s run --force --verbose %s %s %s %s" % (
    ConfigSectionMap("ariba", Config)['ariba_bin'], ConfigSectionMap("ariba", Config)['base_cmd'],
    ConfigSectionMap("ariba", Config)['ariba_amr_db'], forward_paired, reverse_paired, ariba_AMR_dir)
    #print ariba_cmd
...
def ariba_MLST(forward_paired, reverse_paired, output_folder, prefix, logger, Config):
    ...
    ariba_mlst_cmd = "/nfs/esnitkin/bin_group/anaconda3/bin/python %s/%s run --force --verbose %s %s %s %s" % (
    ConfigSectionMap("ariba", Config)['ariba_bin'], ConfigSectionMap("ariba", Config)['base_cmd'],
    ConfigSectionMap("ariba", Config)['ariba_mlst_db'], forward_paired, reverse_paired, ariba_mlst_dir)
